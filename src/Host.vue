...
export default class Host extends Props {
	...
	drawBasicInfo(context: CanvasRenderingContext2D, xPos: number, yPos: number, canvasWidth: number, canvasHeight: number): void {
		...
		this.drawLabel(context, xPos + 660, yPos + 15, 'メンバー数', 40);
		this.drawInput(context, xPos + 660 + context.measureText('メンバー数').width + 40, yPos + 15, this.memberNum.toString(), 40, 'start');
		...
		this.drawLabel(context, xPos + 660, yPos + 90, '加入条件', 40);
		this.drawInput(context, xPos + 660 + context.measureText('加入条件').width + 40, yPos + 90, this.condition, 40, 'start');
		...
		this.drawLabel(context, xPos + 660, yPos + 165, '前クラバト順位', 40);
		if (this.rankingAvailable) {
			this.drawInput(context, xPos + 660 + context.measureText('前クラバト順位').width + 40, yPos + 165, this.ranking + '位', 40, 'start');
		}
		else {
			this.drawInput(context, xPos + 660 + context.measureText('前クラバト順位').width + 40, yPos + 165, '- 位', 40, 'start');
		}
	}
