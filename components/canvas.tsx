  ...
  useEffect(() => {
    ...
    backgroundImage.addEventListener('load', () => {
      ...
      const drawPane = (canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) => {
        ...
        const y = props.state.mode === 'myProfileCard' ? (canvas.height - height) / 2.0 : canvas.height * 3 / 15.0;
        const position = parseInt(props.state.position);
      ...
      const drawLabelAndInput = (canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) => {
        ...
        const y = props.state.mode === 'myProfileCard' ? canvas.height / 6.5 : canvas.height / 4.5;
        ...
        if (props.state.mode === 'myProfileCard') {
          if (props.state.labelBackgroundVisible) {
            context.fillRect(xs[position], y, context.measureText('プレイヤー名').width + 20, fontSize * 1.2);
            context.fillRect(xs[position] + offset, y, context.measureText('プレイヤーLv').width + 20, fontSize * 1.2);
            context.fillRect(xs[position], y + fontSize * 1.5, context.measureText('全キャラ戦力').width + 20, fontSize * 1.2);
            context.fillRect(xs[position] + offset, y + fontSize * 1.5, context.measureText('プレイヤーID').width + 20, fontSize * 1.2);
            context.fillRect(xs[position], y + fontSize * 3.0, context.measureText('クランバトルスコア').width + 20, fontSize * 1.2);
            context.fillRect(xs[position], y + fontSize * 4.5, context.measureText('バトルアリーナ').width + 20, fontSize * 1.2);
            context.fillRect(xs[position] + offset, y + fontSize * 4.5, context.measureText('プリンセスアリーナ').width + 20, fontSize * 1.2);
            context.fillRect(xs[position], y + fontSize * 6.0, context.measureText('ルナの塔').width + 20, fontSize * 1.2);
            context.fillRect(xs[position] + offset, y + fontSize * 6.0, context.measureText('ルナの塔EX').width + 20, fontSize * 1.2);
            context.fillRect(xs[position], y + fontSize * 7.5, context.measureText('コメント').width + 20, fontSize * 1.2);
          }

          context.fillStyle = props.state.labelColor;
          context.fillText('プレイヤー名', xs[position] + 10, y + fontSize);
          context.fillText('プレイヤーLv', xs[position] + 10 + offset, y + fontSize);
          context.fillText('全キャラ戦力', xs[position] + 10, y + fontSize * 2.5);
          context.fillText('プレイヤーID', xs[position] + 10 + offset, y + fontSize * 2.5);
          context.fillText('クランバトルスコア', xs[position] + 10, y + fontSize * 4.0);
          context.fillText('バトルアリーナ', xs[position] + 10, y + fontSize * 5.5);
          context.fillText('プリンセスアリーナ', xs[position] + 10 + offset, y + fontSize * 5.5);
          context.fillText('ルナの塔', xs[position] + 10, y + fontSize * 7.0);
          context.fillText('ルナの塔EX', xs[position] + 10 + offset, y + fontSize * 7.0);
          context.fillText('コメント', xs[position] + 10, y + fontSize * 8.5);

          context.fillStyle = props.state.inputColor;
          context.fillText(props.state.playerName, xs[position] + context.measureText('プレイヤー名   ').width, y + fontSize);
          context.fillText(props.state.playerLevel.toString(), xs[position] + offset + context.measureText('プレイヤーLv   ').width, y + fontSize);
          context.fillText(props.state.strength.toLocaleString(), xs[position] + context.measureText('全キャラ戦力   ').width, y + fontSize * 2.5);
          const playerId = props.state.playerId.substring(0, 3) + " " + props.state.playerId.substring(3, 6) + " " + props.state.playerId.substring(6);
          context.fillText(playerId, xs[position] + offset + context.measureText('プレイヤーID   ').width, y + fontSize * 2.5);
          context.fillText(props.state.score.toLocaleString() + " (" + props.state.scoreDate + ")", xs[position] + context.measureText('クランバトルスコア   ').width, y + fontSize * 4.0);
          context.fillText(props.state.battleArena.toString() + "位", xs[position] + context.measureText('バトルアリーナ   ').width, y + fontSize * 5.5);
          context.fillText(props.state.princessArena.toString() + "位", xs[position] + offset + context.measureText('プリンセスアリーナ   ').width, y + fontSize * 5.5);
          context.fillText(props.state.runa.toString() + "階", xs[position] + context.measureText('ルナの塔   ').width, y + fontSize * 7.0);
          context.fillText(props.state.runaEx.toString(), xs[position] + offset + context.measureText('ルナの塔EX   ').width, y + fontSize * 7.0);
          return;
        }
        ...
        context.fillText(props.state.ranking.toString() + "位 (" + props.state.rankingDate + ")", xs[position] + context.measureText('ランキング   ').width, y + fontSize * 4.0);
      };
      drawLabelAndInput(canvas, context);

      const drawSentence = (canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) => {
        const text = props.state.mode === 'myProfileCard' ? props.state.comment : props.state.guideline;
        const xs = [70.0, canvas.width * 1.0 / 5.0 + 30, canvas.width - (canvas.width * 3.0 / 5.0) - 10.0];
